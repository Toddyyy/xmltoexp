...
MERGED_BASELINE_REPORT = "reports/cumulative_training_merge2_seed42/piece_level_compare.csv"
...
def build_truth_by_level(df: pd.DataFrame, peak_cfg: PeakConfig, beat_unit_fallback: float) -> dict[str, pd.DataFrame]:
    ...
    truth = {}
    for label, target in SPLIT56_TARGETS.items():
...
def main() -> None:
    ...
    run_root = project_root / "outputs/local_runs/cumulative_training_split56_merge2_seed42"
    report_dir = project_root / "reports/cumulative_training_split56_merge2_seed42"
            ...
            rows.append(
                {
                    ...
                    "training_mode": "cumulative_trained_split56_merge2",
    ...
    merged_ref = pd.read_csv(project_root / MERGED_BASELINE_REPORT)
    for row in merged_ref.itertuples(index=False):
        if row.cumulative_level not in SPLIT56_TARGETS:
            continue
        if row.training_mode != "cumulative_trained_merge2":
            continue
        rows.append(
            {
                "piece_id": row.piece_id,
                "cumulative_level": row.cumulative_level,
                "training_mode": "cumulative_trained_merge56_merge2",
                "detector_target": row.detector_target,
    ...
    avg_rows = []
    for label in SPLIT56_TARGETS:
        for training_mode in ("cumulative_trained_merge56_merge2", "cumulative_trained_split56_merge2"):
