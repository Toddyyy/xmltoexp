...
def build_truth_by_level(df: pd.DataFrame, peak_cfg: PeakConfig, beat_unit_fallback: float) -> dict[str, pd.DataFrame]:
    ...
    level6 = component_map["level6"].copy()
    truth["L6"] = level6
...
def run_training(train_script: Path, config: str, target: str, output_dir: Path, *, cumulative: bool) -> None:
    ...
    if cumulative:
        cmd.extend(["--cumulative_merge_tolerance", str(MERGE_TOLERANCE)])
...
def main() -> None:
    ...
    run_root = project_root / "outputs/local_runs/full_split56_hierarchy_seed42"
    report_dir = project_root / "reports/full_split56_hierarchy_seed42"
            ...
            rows.append(
                {
                    ...
                    "cumulative_merge_tolerance": summary.get("cumulative_merge_tolerance"),
                    **metrics,
                }
            )

    detail_path = report_dir / "piece_level_summary.csv"
    ...
    for label in HIERARCHY_TARGETS:
        subset = [row for row in rows if row["hierarchy_level"] == label]
        avg_rows.append(
            {
                "hierarchy_level": label,
                "detector_target": HIERARCHY_TARGETS[label],
    ...
    avg_path = report_dir / "level_mean_summary.csv"
