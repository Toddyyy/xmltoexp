...
OUT_DIR = MIREX / "atepp30_midibert_target_association"
...
def load_source():
    spec = importlib.util.spec_from_file_location("atepp30_corr_source", SOURCE_SCRIPT)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Cannot load {SOURCE_SCRIPT}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["atepp30_corr_source"] = module
...
def main() -> None:
    ...
    src = load_source()
    cfg = src.runner.load_config()
    pieces, labels, _components, _argmax = src.load_sqrtmass_labels()
    ...
    y_binary = (y >= src.EVENT_MIN).astype(np.float32)
