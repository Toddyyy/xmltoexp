...
BEAT_TABLE = METER_AUTO / "outputs" / "atepp_top30_gt25_performance_mixed_by_segment_nonan_beat_table.csv.gz"
LABEL_DIR = METER_AUTO / "beat_data_atepp_top30_gt25_performance_performer_levels"
MANIFEST = METER_AUTO / "outputs" / "atepp_top30_gt25_performance_mixed_by_segment_manifest.csv"
OUT_DIR = MIREX / "atepp30_l2plus_sqrtmass_latest_config"
...
def load_runner():
    spec = importlib.util.spec_from_file_location("atepp30_latest_cnn_runner", RICH_RUN_SCRIPT)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Cannot load {RICH_RUN_SCRIPT}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["atepp30_latest_cnn_runner"] = module
...
def main() -> None:
    ...
    bad = [
        (piece, len(base_features[piece]), len(labels[piece]))
        for piece in pieces
        if len(base_features[piece]) != len(labels[piece])
    ]
    if bad:
    ...
    print(
        f"dataset={DATASET_NAME}; device={device}; pieces={len(pieces)}; "
    ...
    try:
        fold_df, aggregate = runner.run_setting(
            "baseline_cnn",
            cfg,
            pieces,
            labels,
    ...
    agg = pd.DataFrame([{"setting": "baseline_cnn", **aggregate}])
