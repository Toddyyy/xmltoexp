...
EMB_DIR = MIREX / "mazurkabl_midibert_beat_embeddings_meter34"
OUT_DIR = MIREX / "mazurkabl_l2plus_midibert34_cnn_ablation"
EMBED_DIM = 128


def load_base():
    spec = importlib.util.spec_from_file_location("mazurka_l2plus_base_cnn", BASE_SCRIPT)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Cannot load {BASE_SCRIPT}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["mazurka_l2plus_base_cnn"] = module
...
def load_piece_features(pieces: list[str], cfg: dict) -> tuple[dict[str, np.ndarray], list[str]]:
    table_path = Path(cfg["data"]["beat_table_path"])
...
def evaluate_fold(
    ...
    for piece in val_pieces:
        ...
        expected = base.expected_count_from_train_density(train_labels, train_pieces, len(labels[piece]))
