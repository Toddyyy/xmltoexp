...
import sys
ROOT = Path(__file__).resolve().parents[1]  # MERIX SUBMISSION
MIREX = ROOT / "MIREX_Model"
sys.path.insert(0, str(MIREX))

from tokenizer_beat import extract_score_tokens, build_note_features

# Reuse note feature extraction from beat pipeline
...
def load_beat_dyn(file_path: Path):
...
def compute_dyn_curves(df, performer_cols, smooth_window=3, dyn_range=(0.0, 1.0), clip_min=0.0, clip_max=1.0):
    dyn_arrays = {}
    lo, hi = dyn_range
...
def group_analysis_hierarchy(tempo_curve, str_vec, enforce_nested=True):
    """
    Same hierarchy as tempo pipeline; uses valleys on energy to detect boundaries.
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Build per-performer, per-level beat npz from beat_dyn curves."
    )
    parser.add_argument(
        "--beat_dyn_dir",
        default=None,
        help="Directory with *beat_dynNORM.csv (default: <repo>/MazurkaBL-master/beat_dyn).",
    )
    parser.add_argument(
        "--xml_dir",
        default=None,
    ...
    parser.add_argument(
        "--out_dir",
        default=None,
        help="Output dir for *.npz (default: ./beat_data_mazurka_performer_levels_velocity).",
    )
    parser.add_argument(
        "--csv_dir",
        default=None,
        help="Output dir for boundary CSVs (default: ./beat_data_mazurka_performer_velocity).",
    )
    parser.add_argument(
        "--beat_unit",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Rolling mean window for dynamics smoothing (default: 3).",
    )
    parser.add_argument(
        "--dyn_min",
        type=float,
        default=0.0,
        help="Min dynamics for filtering (default: 0).",
    )
    parser.add_argument(
        "--dyn_max",
        type=float,
        default=1.0,
        help="Max dynamics for filtering (default: 1).",
    )
    parser.add_argument(
        "--clip_min",
        type=float,
        default=0.0,
        help="Clip dynamics to min (default: 0).",
    )
    parser.add_argument(
        "--clip_max",
        type=float,
        default=1.0,
        help="Clip dynamics to max (default: 1).",
    )
    parser.add_argument(
    ...
    beat_dyn_dir = (
        Path(args.beat_dyn_dir)
        if args.beat_dyn_dir
        else repo_root / "MazurkaBL-master" / "beat_dyn"
    )
    xml_dir = (
        Path(args.xml_dir)
        if args.xml_dir
        else repo_root / "MazurkaBL-master" / "xml_scores"
    )
    out_dir = (
        Path(args.out_dir)
        if args.out_dir
        else Path(__file__).resolve().parent / "beat_data_mazurka_performer_levels_velocity"
    )
    csv_dir = (
        Path(args.csv_dir)
        if args.csv_dir
        else Path(__file__).resolve().parent / "beat_data_mazurka_performer_velocity"
    )

    if not beat_dyn_dir.exists():
        raise FileNotFoundError(f"beat_dyn_dir not found: {beat_dyn_dir}")
    ...
    for file_path in sorted(beat_dyn_dir.glob("*beat_dynNORM.csv")):
        ...
        df_bt, performer_cols = load_beat_dyn(file_path)
        if not performer_cols:
            skipped.append((file_path.name, "no_performers"))
            continue

        dyn_arrays = compute_dyn_curves(
            df_bt,
            performer_cols,
            smooth_window=args.smooth_window,
            dyn_range=(args.dyn_min, args.dyn_max),
        ...
        for perf_id, curve in dyn_arrays.items():
        ...
        if performer_count == 0:
            skipped.append((file_path.name, "no_curves"))
