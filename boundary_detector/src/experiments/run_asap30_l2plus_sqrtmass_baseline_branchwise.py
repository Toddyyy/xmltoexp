...
BRANCHWISE_SCRIPT = EXPERIMENTS / "run_mazurkabl_l2plus_branchwise_bottleneck_cnn.py"

TOP_N = int(os.environ.get("ASAP_TOP_N", "30"))
DATASET_NAME = f"asap{TOP_N}"
LABEL_ROOT = DATA / "labels" / f"{DATASET_NAME}_tempo_boundary_labels"
LABEL_DIR = LABEL_ROOT / f"beat_data_asap_top{TOP_N}_performer_levels"
BEAT_TABLE = LABEL_ROOT / f"asap_top{TOP_N}_beat_table.csv.gz"
MANIFEST = LABEL_ROOT / f"asap_top{TOP_N}_manifest.csv"
BOUNDARY_SUMMARY = LABEL_ROOT / f"asap_top{TOP_N}_boundary_summary.csv"
RICH_DIR = DATA / "features" / f"{DATASET_NAME}_midibert_rich_beat_features"
OUT_DIR = RESULTS / f"{DATASET_NAME}_l2plus_sqrtmass_baseline_branchwise"
